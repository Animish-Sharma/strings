...
**Relationship to `witsoc-2`.** `../witsoc-2/` is the same architecture rebuilt
...
`witsoc-2` rebuild, so the two skills describe one architecture. The roles are
...
declares a status ceiling that passing the gate never lifts. `witsoc-2`
...
5. **Per-pack conformance, plus one frame suite.** Each domain pack should be
   ...
   `witsoc-2` rebuild implements both (`domains/_mock/`,
