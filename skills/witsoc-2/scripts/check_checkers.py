...
Ten `check_*.py` scripts guard this tree and not one of them had a test. The
...
COPY = ["scripts", "schemas", "references", "domains", "explorer", "generator",
        "researcher", "SKILL.md", "ARCHITECTURE.md"]
...
PLANTS = [
    ...
    {"name": "doctrine commands (dead command)", "script": "check_doctrine_commands.py",
     "plant": lambda r: (r / "domains/maths/doctrine/explorer.md").write_text(
         (r / "domains/maths/doctrine/explorer.md").read_text()
