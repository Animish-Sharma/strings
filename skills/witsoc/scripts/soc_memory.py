...
def render(soc: dict) -> str:
    ...
    for failure in soc.get("failed_approaches", []):
        ...
        if failure.get("failure_signature"):
            out.append(f"    signature: {failure['failure_signature']}")
        if failure.get("consecutive_failures"):
            out.append(f"    streak: {failure['consecutive_failures']}")
...
def main() -> int:
    ...
    for name in ("current", "insight", "failure", "decide", "attribute", "check",
        ...
        if name == "failure":
            ...
            p.add_argument("--signature"); p.add_argument("--streak", type=int)
        if name == "decide":
    ...
    try:
        ...
        if args.cmd == "failure":
            ...
            if args.evidence:
                entry["evidence_ref"] = args.evidence
            if args.signature:
                entry["failure_signature"] = args.signature
            if args.streak is not None:
                if args.streak < 1:
                    print("REFUSED: --streak must be at least 1", file=sys.stderr)
                    return 1
                entry["consecutive_failures"] = args.streak
