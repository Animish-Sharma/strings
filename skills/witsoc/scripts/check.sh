...
# Usage:
#   check.sh              # everything
#   check.sh --fast       # runtime contract and synthetic core only
...
MODE="all"; ONLY=""
if [[ $# -eq 0 && -f "$SKILL_ROOT/.witsoc-manifest.json" ]]; then
  # Installed releases omit development fixtures by policy. Their default is
  # the runtime suite; the source tree still defaults to the complete suite.
  MODE="fast"
fi
while [[ $# -gt 0 ]]; do
  case "$1" in
...
run_step() {
  ...
  if "$@" >/tmp/witsoc-check.$$ 2>&1; then
    ...
    printf "  %s  %-22s %s\n" "$mark" "$label" \
      "$(grep -v '^NOT_RUN_COUNT=' /tmp/witsoc-check.$$ | tail -1)"
    ...
    if (( code == 3 )); then
      printf "  ----  %-22s %s\n" "$label" "$(tail -1 /tmp/witsoc-check.$$)"
      NOT_RUN=$(( NOT_RUN + 1 ))
    else
      printf "  FAIL  %-22s\n" "$label"
      sed 's/^/          /' /tmp/witsoc-check.$$ | tail -12
      FAILED=1
    fi
  fi
  TOTAL_MS=$(( TOTAL_MS + elapsed_ms ))
  STEPS=$(( STEPS + 1 ))
  rm -f /tmp/witsoc-check.$$
}
...
if [[ "$MODE" == "fast" ]]; then
  if [[ -f "$SKILL_ROOT/.witsoc-manifest.json" ]]; then
    run_step "runtime contract" python3 "$SCRIPTS_DIR/witsoc.py" doctor
    run_step "runtime core"     python3 "$SCRIPTS_DIR/witsoc.py" self-test
    run_step "resolution"       python3 "$SCRIPTS_DIR/resolve_domain.py" --self-test
  else
    run_step "runtime contracts" python3 "$SCRIPTS_DIR/generate_contracts.py" --check
    [[ -f "$SCRIPTS_DIR/package_domains.py" ]] && \
      run_step "package registry" python3 "$SCRIPTS_DIR/package_domains.py" registry
    run_step "runtime core"     python3 "$SCRIPTS_DIR/witsoc.py" self-test
  ...
  run_step "skill discovery" python3 "$SCRIPTS_DIR/check_skill_discovery.py"
  run_step "delivery"        python3 "$SCRIPTS_DIR/check_delivery.py"
  ...
  run_step "runtime sync"    python3 "$SCRIPTS_DIR/sync_runtime.py" --self-test
...
for name in $(packs); do
  ...
  while IFS= read -r held; do
    ...
    if python3 "$held" >/tmp/witsoc-ext.$$ 2>&1; then
      printf "  ok    %-22s %s\n" "$label" "$(tail -1 /tmp/witsoc-ext.$$)"
      STEPS=$(( STEPS + 1 ))
    elif grep -qE "arguments are required|NOT_RUN" /tmp/witsoc-ext.$$; then
      printf "  ----  %-22s needs outside material: %s\n" "$label" \
        "$(grep -oE 'required: --[a-z-]+|NOT_RUN[^\n]*' /tmp/witsoc-ext.$$ | head -1 | cut -c1-70)"
      NOT_RUN=$(( NOT_RUN + 1 ))
    else
      printf "  FAIL  %-22s\n" "$label"
      sed 's/^/          /' /tmp/witsoc-ext.$$ | tail -8
      FAILED=1
    fi
    rm -f /tmp/witsoc-ext.$$
