#!/usr/bin/env python3
"""Admit one candidate artifact through every generic frame packet.
...
This closes that artifact-admission gap. One command drives:
...
This is not the campaign-wide loop for a domain with a canonical frontier.
Load the selected pack's `campaign_start` doctrine and use the driver it names;
calling this once per endpoint does not reconstruct or advance that frontier.

Usage:
    campaign.py artifact --claim <claim.json> --artifact <path> --tier <name>
...
`run` remains a compatibility alias for `artifact`.
...
class Campaign:
    ...
    def execute(self, claim_path: Path, artifact: Path, tier: str,
        ...
        if risk.get("repeat_risk") == "HIGH":
            ...
            repeat_match = (risk.get("matches") or [{}])[0]
            repeat_obstruction = str(
                repeat_match.get("blocker") or "a recorded failure recurred unchanged"
            )
            packet = self.escalation_packet(
                claim, target, state, pack, plan,
                {"escalated_by": "repeat gate",
                 "obstruction": repeat_obstruction,
                 "signature": repeat_match.get("failure_signature"),
                 "streak": repeat_match.get("consecutive_failures")})
            soc = soc_memory.load(soc_path)
            soc["current"]["obstruction"] = repeat_obstruction
            soc_memory.save(soc_path, soc)
        ...
        if verdict != "pass":
            ...
            if code not in {0, 3}:
                self.step("record failure", False, (out + err).strip()[:200])
                return self.report("STOPPED", pack=pack, ceiling=ceiling, receipt=receipt)
            assessment = json.loads(out) if out.strip().startswith("{") else {}
            if (code == 3) != bool(assessment.get("escalate")):
                self.step("record failure", False, "failure-ledger exit and JSON assessment disagree")
                return self.report("STOPPED", pack=pack, ceiling=ceiling, receipt=receipt)
            signature = assessment.get("recorded")
            streak = assessment.get("consecutive_failures")
            obstruction = str(
                claim.get("active_obstruction")
                or (claim.get("frozen_conditions") or {}).get("active_obstruction")
                or receipt.get("failure_class")
                or verdict
            )[:300]

            # Two records of the same failure are complementary. The ledger
            # owns the normalized signature and streak; working memory owns the
            # mathematical blocker and revival condition. Persist both so the
            # repeat gate can hand off the actual obstruction rather than the
            # generic instruction not to rerun an unchanged artifact.
            soc_cmd = [
                sys.executable, str(HERE / "soc_memory.py"), "failure",
                "--soc", str(soc_path), "--method", f"{pack}:{tier}",
                "--statement", claim.get("exact_statement", "")[:300],
                "--blocker", obstruction,
                "--do-not-repeat", f"route {pack}:{tier} with failure signature {signature}",
                "--revival", "new evidence that clears the named obstruction or a different semantic route",
            ]
            if signature:
                soc_cmd += ["--signature", str(signature)]
            if isinstance(streak, int) and streak > 0:
                soc_cmd += ["--streak", str(streak)]
            code, soc_out, soc_err = run(soc_cmd)
            if code != 0:
                self.step("working memory", False, (soc_out + soc_err).strip()[:200])
                return self.report("STOPPED", pack=pack, ceiling=ceiling, receipt=receipt)
            soc = soc_memory.load(soc_path)
            soc["current"]["obstruction"] = obstruction
            soc_memory.save(soc_path, soc)
    ...
    def report(self, outcome: str, **extra) -> dict:
        ...
        if self._soc_path is not None and self._soc_path.is_file():
            soc = soc_memory.load(self._soc_path)
            soc["run_status"] = (
                "DONE" if outcome in {"ADMITTED", "REFUSED"} else "STUCK"
            )
            receipt = extra.get("receipt") or {}
            obstruction = receipt.get("failure_class")
            if obstruction:
                soc["current"]["obstruction"] = str(obstruction)
            soc_memory.save(self._soc_path, soc)
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        pass_soc = json.loads((run_dir / "soc.json").read_text(encoding="utf-8"))
        if pass_soc.get("run_status") != "DONE":
            failures.append("an admitted artifact left working memory RUNNING")
        ...
        failed_soc = json.loads((run_dir / "soc.json").read_text(encoding="utf-8"))
        remembered = (failed_soc.get("failed_approaches") or [{}])[-1]
        memory_complete = (
            failed_soc.get("run_status") == "STUCK"
            and bool(remembered.get("blocker"))
            and bool(remembered.get("failure_signature"))
            and isinstance(remembered.get("consecutive_failures"), int)
        )
        print(f"  {'ok  ' if memory_complete else 'MISS'}  terminal working memory names the "
              "blocker, signature, and streak")
        if not memory_complete:
            failures.append("escalation left incomplete or RUNNING working memory")
        ...
        gap_soc = json.loads((gap_dir / "soc.json").read_text(encoding="utf-8"))
        if gap_soc.get("failed_approaches"):
            failures.append("an incomplete check polluted working memory with a failed approach")
...
def main() -> int:
    ...
    r = sub.add_parser(
        "artifact", aliases=["run"],
        help="admit one artifact; `run` is a compatibility alias, not a campaign-wide frontier",
    )
