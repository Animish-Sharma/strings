...
def main() -> int:
    ...
    if registry.is_file():
        ...
        manifest = SKILL_ROOT / ".witsoc-manifest.json"
        policy_path = SKILL_ROOT / "references" / "runtime_release.json"
        if manifest.is_file() and policy_path.is_file():
            try:
                patterns = json.loads(policy_path.read_text(encoding="utf-8"))["exclude_patterns"]
                intentionally_excluded = {
                    pack
                    for pack in listed
                    if any(
                        fnmatch.fnmatch(f"domains/{pack}/domain.json", pattern)
                        for pattern in patterns
                    )
                }
            except (OSError, json.JSONDecodeError, KeyError, TypeError):
                intentionally_excluded = set()
        facts["packs_intentionally_excluded"] = sorted(intentionally_excluded)
        for pack in on_disk:
            if pack not in listed:
                problems.append(f"pack {pack!r} exists and the registry does not list it")
        for pack in sorted(listed - set(on_disk) - intentionally_excluded):
