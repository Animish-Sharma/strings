...
def validate(
    ...
    if reference:
        if not isinstance(reference, str) or not reference.startswith("#/"):
            errors.append(f"{path}: unsupported schema reference {reference!r}")
            return
        target: Any = root
        try:
        ...
        if not isinstance(target, dict):
            errors.append(f"{path}: schema reference {reference!r} is not an object")
            return
    ...
    if "allOf" in schema:
        for branch in schema["allOf"]:
            validate(node, branch, path, errors, root)
    ...
    if "oneOf" in schema:
        ...
        for branch in schema["oneOf"]:
            current: list[str] = []
            validate(node, branch, path, current, root)
            branch_errors.append(current)
            if not current:
                matches += 1
        if matches != 1:
            detail = branch_errors[0][0] if matches == 0 and branch_errors and branch_errors[0] else ""
            errors.append(
                f"{path}: must satisfy exactly one schema branch, matched {matches}"
                + (f"; first mismatch: {detail}" if detail else "")
            )
        return
    ...
    if isinstance(node, list):
        ...
        if max_items is not None and len(node) > max_items:
            errors.append(f"{path}: allows at most {max_items} item(s), got {len(node)}")
        if schema.get("uniqueItems"):
            encoded = [json.dumps(item, sort_keys=True, separators=(",", ":")) for item in node]
            if len(encoded) != len(set(encoded)):
                errors.append(f"{path}: array items must be unique")
        item_schema = schema.get("items")
        if isinstance(item_schema, dict):
            for index, item in enumerate(node):
                validate(item, item_schema, f"{path}[{index}]", errors, root)

    if isinstance(node, dict):
        min_properties = schema.get("minProperties")
        if min_properties is not None and len(node) < min_properties:
            errors.append(f"{path}: needs at least {min_properties} field(s), got {len(node)}")
        ...
        for key, value in node.items():
            if key in properties:
                validate(value, properties[key], f"{path}.{key}", errors, root)
                ...
                validate(value, extra, f"{path}.{key}", errors, root)
