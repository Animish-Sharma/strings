...
All checks must pass before the pack is registered. A remotely installable pack
also needs a project under `packages/`; `package_domains.py registry --write`
copies only its selection contract and pinned payload identity into the thin
core. The wheel owns the remaining domain bytes. Runtime installation is
binary-only, dependency-free, compatibility-checked, digest-checked, and
atomically activated before full resolution.

`domains/_mock/` is a minimal
